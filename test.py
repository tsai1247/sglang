...
cross_encoder_model = CrossEncoder('./cross_encoder_finetuned', num_labels=1)

def predict_best_candidate(input_text, candidates_list):
    candidates_values = [c["Token String"] for c in candidates_list]
    # 準備 100 對 (input, cand)
    pairs = [[input_text, cand] for cand in candidates_values]
    
    # 讓模型一次評分所有對 (GPU 上可以 batch 處理，很快)
    scores = cross_encoder_model.predict(pairs) # 返回 array([0.9, 0.1, 0.05 ...])
    
    # 找出最高分的 index
    # print("scores", scores)
    best_idx = scores.argmax()
    return candidates_values[best_idx], candidates_list[best_idx]['Token ID'], scores[best_idx]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for step in range(NUM_STEPS):
            ...
            next_token_str, next_token_id, _score = predict_best_candidate(prompt, candidates_list)
